...
- **定义（Def）**：对实体之结构、约束、行为和关系的规范性描述，包含字段定义、状态机、合约等；类似于面向对象中的**类（Class）**；
- **关系（Relation）**：实体之间的有向或无向连接，可具有时效窗口和基数约束；
- **类型（Type）**：与 Def 协作构成实体行为的特化机制，提供层级分类和继承能力，可扩展或覆盖 Def 的合约与约束；类似于面向对象中的**接口（Interface）**，但支持层级继承；
- **合约（Contract）**：嵌入存储系统写入路径的可执行行为；
- **版本（Version）**：实体在某一时刻状态的不可变快照，形成历史链条。
