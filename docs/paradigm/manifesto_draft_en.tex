...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small Data-First Ontology}
...
% Bibliography (numbers in square brackets)
\makeatletter
...
\title{Data-First Ontology: The World Model Resides in Data, Not in Weights}
\author{Zhanbo Li / Atlas Lee\thanks{DAO is derived from the  Chinese philosophical concept ``道'' (Tao/Dao), signifying the origin  and governing principle of all things. It is unrelated to DAO  (Decentralized Autonomous Organization) in the blockchain context.}}
\affil{DAO Project}
\date{May 2026}
...
\section{Introduction: The Crisis of World
Models}\label{introduction-the-crisis-of-world-models}

\subsection{The Default Assumption}\label{the-default-assumption}
...
\cite{kaplan2020,hoffmann2022} are interpreted not merely as empirical regularities of loss
...
\subsection{Four Structural Crises}\label{four-structural-crises}
...
factually incorrect statements with high confidence \cite{ji2023}. The root
...
of training. Retrieval-Augmented Generation (RAG) \cite{lewis2020} and
fine-tuning \cite{hu2022} are techniques that attempt to address this
...
model, and carries the risk of catastrophic forgetting \cite{mccloskey1989}. The
...
which other tokens \cite{bahdanau2015}, but they do not reveal why the model
...
may introduce new errors elsewhere \cite{meng2022}. There exists no mechanism
...
\section{The Problem: Implicit vs.~Explicit World
Models}\label{the-problem-implicit-vs.-explicit-world-models}

\subsection{What Is a World Model?}\label{what-is-a-world-model}
...
following capabilities \cite{ha2018,lecun2022}:
...
\subsection{Implicit World Model}\label{implicit-world-model}
...
\subsection{Explicit World Model}\label{explicit-world-model}
...
\subsection{Temporal and Immutable
Databases}\label{temporal-and-immutable-databases}

\textbf{Datomic} \cite{hickey2012} is the closest precursor to the ideas in this
...
\textbf{Event Store} \cite{young2011} and \textbf{Temporal Tables} (SQL:2011)
\cite{kulkarni2012} have also made important contributions along the temporal
...
\subsection{Knowledge Representation and Knowledge
Graphs}\label{knowledge-representation-and-knowledge-graphs}

\textbf{RDF/OWL/SPARQL} \cite{w3c-rdf2014,w3c-owl2012,w3c-sparql2013} constitute the technology stack
...
domain \cite{hogan2021} further reveals structural gaps in existing knowledge
representation systems regarding dynamic evolution and multi-modal
unification.

\textbf{TypeDB} \cite{ioannou2023} is a typed hyper-relational knowledge graph
...
\textbf{Property Graph} (e.g., Neo4j \cite{webber2015}, GQL \cite{francis2024}) organizes
...
graph database models \cite{angles2008} notes that existing graph databases
exhibit fundamental limitations in type systems and behavioral
semantics.

\subsection{Large Language Models and Structured
Data}\label{large-language-models-and-structured-data}

\textbf{GraphRAG} \cite{edge2024}, proposed by Microsoft Research, is
...
queries. LightRAG \cite{guo2024} optimizes indexing and retrieval efficiency
on this basis, adopting a dual-tier retrieval system. FastGraphRAG
\cite{fastgraphrag2024} further accelerates graph traversal using personalized
PageRank.
...
\textbf{MemGPT} \cite{packer2023}, proposed by UC Berkeley, applies the virtual
...
\textbf{MCP (Model Context Protocol)} \cite{anthropic2025} is an open protocol aimed
...
\textbf{Neuro-symbolic AI} \cite{garcez2023} is dedicated to combining the
...
Provers \cite{rocktaschel2017} and Logic Tensor Networks \cite{serafini2016} attempted to embed
logical rules into neural networks. Recent work such as AlphaProof
\cite{alphaproof2024} demonstrates the potential of neuro-symbolic integration in
...
\subsection{Summary of Distinctions from Existing
...
\section{Core Argument: Data-First
Ontology}\label{core-argument-data-first-ontology}

\subsection{Three Propositions}\label{three-propositions}
...
\subsection{Bootstrapping: A Self-Describing
...
\subsection{Time as Ontology, Not
...
\subsection{Behavior Embedded in the Storage
...
\subsection{DaoQL-Edu: Pedagogical
...
\subsection{Full-Version Architecture
...
\section{Philosophy: Computational Ontological
Realism}\label{philosophy-computational-ontological-realism}

\subsection{Dialogue with Existing Philosophical
...
\subsection{Other Future Work}\label{other-future-work}
...
\subsection{Limitations of Measurement
...
\subsection{Meta-Tool Layer Not
...
\subsection{Symbol Grounding Problem
...
\subsection{Not a General-Purpose
...
\subsection{Symbol Grounding}\label{symbol-grounding}
...
\subsection{Evolution Consistency}\label{evolution-consistency}
...
\subsection{Invitation}\label{invitation}
...
\bibitem{hoffmann2022} J.~Hoffmann et al., ``Training compute-optimal large language
...
\bibitem{lewis2020} P.~Lewis et al., ``Retrieval-augmented generation for
  knowledge-intensive NLP tasks,'' in \emph{NeurIPS}, vol.~33, pp.~9459--9474, 2020.

\bibitem{hu2022} E.J.~Hu et al., ``LoRA: Low-rank adaptation of large language
  models,'' in \emph{ICLR}, 2022.

\bibitem{mccloskey1989} M.~McCloskey and N.J.~Cohen, ``Catastrophic interference in
  connectionist networks: The sequential learning problem,''
  \emph{Psychology of Learning and Motivation}, vol.~24, pp.~109--165, 1989.

\bibitem{bahdanau2015} D.~Bahdanau, K.~Cho, and Y.~Bengio, ``Neural machine
  translation by jointly learning to align and translate,'' in \emph{ICLR}, 2015.

\bibitem{meng2022} K.~Meng et al., ``Locating and editing factual associations in
  GPT,'' in \emph{NeurIPS}, 2022.

\bibitem{ha2018} D.~Ha and J.~Schmidhuber, ``World models,''
  \emph{arXiv preprint arXiv:1803.10122}, 2018.

\bibitem{lecun2022} Y.~LeCun, ``A path towards autonomous machine intelligence,''
...
\bibitem{w3c-rdf2014} W3C, ``RDF 1.1 Concepts and Abstract Syntax,''
  W3C Recommendation, 2014.

\bibitem{w3c-owl2012} W3C, ``OWL 2 Web Ontology Language Document Overview,''
  W3C Recommendation, 2012.

\bibitem{w3c-sparql2013} W3C, ``SPARQL 1.1 Query Language,''
...
\bibitem{webber2015} J.~Webber, ``Graph Databases: New Opportunities for Connected
  Data,'' \emph{O'Reilly Media}, 2015.

\bibitem{francis2024} N.~Francis et al., ``GQL: A query language for property
  graphs,'' in \emph{ACM SIGMOD}, 2024.

\bibitem{edge2024} D.~Edge et al., ``From Local to Global: A Graph RAG Approach
  to Query-Focused Summarization,''
  \emph{arXiv preprint arXiv:2404.16130}, 2024.

\bibitem{guo2024} Z.~Guo et al., ``LightRAG: Simple and Fast Retrieval-Augmented
  Generation,'' \emph{arXiv preprint arXiv:2410.05779}, 2024.

\bibitem{fastgraphrag2024} FastGraphRAG.AI, ``FastGraphRAG: Accelerated knowledge
  intensive reasoning,'' 2024.

\bibitem{packer2023} C.~Packer et al., ``MemGPT: Towards LLMs as Operating
  Systems,'' \emph{arXiv preprint arXiv:2310.08560}, 2023.

\bibitem{anthropic2025} Anthropic, ``Model Context Protocol Specification,'' 2025.

\bibitem{garcez2023} A.S.~Garcez and L.C.~Lamb, ``Neurosymbolic AI: The 3rd
  Wave,'' \emph{Artificial Intelligence Review}, 2023.

\bibitem{rocktaschel2017} T.~Rockt\"{a}schel and S.~Riedel, ``End-to-end differentiable
  proving,'' in \emph{NeurIPS}, 2017.

\bibitem{serafini2016} L.~Serafini and A.S.~Garcez, ``Logic Tensor Networks: Deep
  Learning and Logical Reasoning from Data and Knowledge,''
  \emph{arXiv preprint arXiv:1606.04422}, 2016.

\bibitem{alphaproof2024} AlphaProof Team, ``AI achieves silver-medal standard solving
  international mathematical olympiad problems,'' \emph{DeepMind Blog}, 2024.
...
\bibitem{hogan2021} A.~Hogan et al., ``Knowledge graphs,''
  \emph{ACM Computing Surveys}, vol.~54, no.~4, pp.~1--37, 2021.

\bibitem{guarino1995} N.~Guarino, ``Formal ontology, conceptual analysis and
  knowledge representation,'' \emph{International Journal of Human-Computer
  Studies}, vol.~43, no.~5-6, pp.~625--640, 1995.
